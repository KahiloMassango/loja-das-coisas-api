package org.example.loja_das_coisas_api.product.repository.util
...
class ProductSpecification {
    ...
    private fun storeIsActive(): Specification<Product> =
        Specification { root, _, cb ->
            val storeJoin = root.join<StoreProfile, Product>("store", JoinType.LEFT)
            cb.isTrue(storeJoin.get("active"))
        }

    private fun byStoreId(storeId: UUID): Specification<Product> =
        Specification { root, _, cb ->
            val storeJoin = root.join<StoreProfile, Product>("store")
